...
/**
 * This servlet is used to find a beer by brewery name. It takes in a string
 * containing a name. It returns a complete beer list with all appropriate
 * information.
 *
 * Getting by brewery name is a get request and is the only operation in the
 * servlet.
 *
 * @author Christian Witchger
 *
 */
@WebServlet("/beerByBrewery")
public class BeerByBrewery extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (valid) {
			try {
				...
				statement = connection.prepareStatement("SELECT B.id_beer, B.name, B.icon, B.abv, B.ibu, B.barArea, "
						...
						+ "Br.id_location = L.id_location and Br.name LIKE '%" + brewery + "%' ORDER BY B.name ASC");
		...
		innerObject.put("brewery", brewery);
