...
/**
 * This servlet is used to find a beer by ABV. It takes in a string
 * representation of a number. It returns a complete beer list with all
 * appropriate information
 *
 * Getting by ABV is a get request and is the only operation in the servlet.
 *
 * @author Christian Witchger
 *
 */
@WebServlet("/beerByABV")
public class BeerByABV extends HttpServlet {
		...
		if (valid) {
			try {
				...
				statement = connection.prepareStatement("SELECT B.id_beer, B.name, B.icon, B.abv, B.ibu, B.barArea,"
						...
						+ "Br.id_location = L.id_location and B.abv = 48 ORDER BY B.name ASC");
				statement.setString(1, abv);
