...
/**
 * This servlet is used to get a beer list It takes in a string containing a
 * list id. It returns a complete beer list with all appropriate information.
 *
 * Getting by list id is a get request and is the only operation in the servlet.
 *
		...
		if (valid) {
			try {
				...
				statement = connection.prepareStatement("SELECT B.id_beer, B.name, B.icon, B.abv, B.ibu, B.barArea, "
				        ...
				        + "WHERE Bl.id_beer = B.id_beer and B.id_Brewry = Br.id_Brewry and "
				        ...
				        + "Br.id_location = L.id_location and BL.listNumber = " + listId + " ORDER BY B.name ASC");
