...
/**
 * This servlet is used to find a beer by style of beer. It takes in a string
 * containing a style. It returns a complete beer list with all appropriate
 * information.
 *
 * Getting by style of beer is a get request and is the only operation in the
 * servlet.
 *
 * @author Christian Witchger
 *
 */
@WebServlet("/beerByStyle")
public class BeerByStyle extends HttpServlet {
		...
		if (valid) {
			try {
				...
				statement = connection.prepareStatement("SELECT B.id_beer, B.name, B.icon, B.abv, B.ibu, B.barArea, "
				        ...
				        + "Br.id_location = L.id_location and GBT.value LIKE '%" + style + "%' OR SBT.value LIKE '%"
				        + style + "%' ORDER BY B.name ASC");
